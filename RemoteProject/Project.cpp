...
LRESULT CRemoteProject::OnProcess(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& bHandled)
{
   ...
   // Try to obtain the semaphore (NOTE: WinNT specific API)
...
void CRemoteProject::_InitializeData()
{
   ...
   s.LoadString(IDS_CAPTION_COMPILELOG);
   dwStyle = WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
   m_viewCompileLog.Init(&m_CompileManager.m_ShellManager);
   m_viewCompileLog.Create(m_wndMain, CWindow::rcDefault, s, dwStyle, WS_EX_CLIENTEDGE);
   _pDevEnv->AddDockView(m_viewCompileLog, IDE_DOCK_HIDE, CWindow::rcDefault);

   // Create the Debug Log view
   COLORREF clrBack = BlendRGB(::GetSysColor(COLOR_WINDOW), RGB(0,0,0), 5);
   s.LoadString(IDS_CAPTION_DEBUGLOG);
   dwStyle = WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
   m_viewDebugLog.Init(&m_DebugManager.m_ShellManager);
   m_viewDebugLog.Create(m_wndMain, CWindow::rcDefault, s, dwStyle, WS_EX_CLIENTEDGE);
   ...
   m_viewClassTree.Init(this);
   m_viewClassTree.Create(m_wndMain, CWindow::rcDefault, s, dwStyle, 0);
